...
class MesuresDB:
    ...
    def supprimer_etude(self, study_instance_uid):
        ...
        self.curseur.execute(
            "DELETE FROM metastase WHERE SeriesInstanceUID IN (SELECT SeriesInstanceUID FROM series WHERE StudyInstanceUID=?)",
            (study_instance_uid,))
    ...
    ## MÉTASTASE
    def ajouter_metastase(self, roi_number, series_instance_uid, nom_metastase, volume, diametre, slide_debut,
                          slide_fin, color):
        self.curseur.execute(
            "INSERT INTO metastase (roiNumber, SeriesInstanceUID, nom_metastase, volume, diametre, slideDebut, slideFin, color) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    def get_metastases(self):
        ...
        return [{"id": met[0], "roiNumber": met[1], "series_instance_uid": met[2], "nom_metastase": met[3],
                 "volume": met[4], "diametre": met[5], "slice_debut": met[6], "slice_fin": met[7], "color": met[8]} for
                met in metastases]
    ...
    def get_metastases_from_serie(self, series_instance_uid):
        ...
        metastases = self.curseur.fetchall()
        return [{"id": met[0], "roiNumber": met[1], "nom_metastase": met[3], "volume": met[4], "diametre": met[5],
                 "slice_debut": met[6], "slice_fin": met[7], "color": met[8]} for met in metastases]
    ...
    def supprimer_metastase_from_serie(self, series_instance_uid, roi_number):
        self.curseur.execute("DELETE FROM metastase WHERE SeriesInstanceUID=? AND roiNumber=?",
                             (series_instance_uid, roi_number))
        self.connexion.commit()

    def renommer_metastase_from_serie(self, series_instance_uid, roi_number, new_name):
        self.curseur.execute("UPDATE metastase SET nom_metastase = ? WHERE SeriesInstanceUID = ? AND roiNumber = ?",
                             (new_name, series_instance_uid, roi_number))
        self.connexion.commit()
